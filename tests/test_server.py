...
def test_dashboard_restart_socket_failure(monkeypatch, tmp_path):
    """A failed transport must not be reported as a successful Docker restart."""
    monkeypatch.chdir(tmp_path)
    real_run = server.run_command

    def isolated_run(command, timeout):
        command = list(command)
        command[command.index("--unix-socket") + 1] = "missing-docker.sock"
        return real_run(command, timeout)

    monkeypatch.setattr(server, "run_command", isolated_run)
    assert server.update_inverter_dashboard("v2.0.0") == (False, "Failed: see server logs")
